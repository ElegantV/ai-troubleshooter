  ...
  <div class="wrap" v-if="auth.ready">
    <header class="hero">
      ...
      <div class="stats" v-if="meta.data">
        <div class="stat"><b>{{ meta.data.tables }}</b><span>表</span></div>
        <div class="stat"><b>{{ meta.data.jobs }}</b><span>作业</span></div>
        <div class="stat"><b>{{ meta.data.lineageEdges }}</b><span>血缘边</span></div>
        <div class="stat"><b>{{ meta.data.tickets }}</b><span>历史工单</span></div>
        <div class="stat"><b>{{ meta.data.cases }}</b><span>案例</span></div>
        <div class="stat stat-date"><b>{{ meta.data.latestRunDate }}</b><span>最新跑批日</span></div>
      </div>
      <div class="alerts" v-if="meta.data && meta.data.abnormalJobs.length">
        <button class="alerts-toggle" :class="{ open: alertsOpen }" @click="alertsOpen = !alertsOpen">
        ...
        <div class="alerts-list" v-if="alertsOpen">
          <span v-for="j in meta.data.abnormalJobs" :key="j.job_name" class="alerts-item">{{ j.job_name }} · {{ j.final_status }}</span>
        </div>
      </div>
    </header>
    ...
    <main class="main">
      ...
      <el-tabs v-model="ui.activeTab" class="main-tabs">
        ...
        <el-tab-pane label="排查历史" name="queries" lazy><QueriesView v-if="ui.activeTab === 'queries'" /></el-tab-pane>
      </el-tabs>
    </main>
...
:root {
  ...
  --r-xl: 0.875rem; --r-2xl: 1.125rem; --r-pill: 62.5rem;
...
::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 0.25rem; }
...
.brand { display: flex; gap: var(--sp-3); align-items: flex-start; min-width: 0; flex: 1 1 18rem; }
...
.hero .ver {
  ...
  margin-left: 0.375rem; font-weight: 500; vertical-align: 0.125rem;
}
.hero .sub { color: #dbeafe; font-size: var(--fs-body); margin-top: var(--sp-2); }
...
.who-role, .who-sys {
  ...
  font-size: var(--fs-cap); background: rgba(255, 255, 255, 0.2);
  padding: 0.0625rem 0.5rem; border-radius: var(--r-pill);
}
.who-ico { font-size: 0.75em; }
.who .el-link.who-logout { color: #dbeafe; font-size: var(--fs-body); }
...
.stat b {
  font-size: var(--fs-stat); font-weight: 700; line-height: 1;
  font-variant-numeric: tabular-nums;
}
.stat span { font-size: var(--fs-cap); color: #bfdbfe; }
/* 日期不是计数，降一级避免被当成大数字 */
.stat-date b { font-size: var(--fs-strong); font-weight: 600; letter-spacing: 0.01em; }
...
.alerts-toggle {
  ...
  background: rgba(220, 38, 38, 0.85); color: #fff; font-size: var(--fs-cap);
  padding: var(--sp-1) var(--sp-3); border-radius: var(--r-pill);
  transition: background 0.15s ease;
}
.alerts-toggle:hover { background: rgba(220, 38, 38, 1); }
.alerts-toggle.open .alerts-chev { transform: rotate(180deg); }
.alerts-ico { font-size: 0.875rem; }
.alerts-chev { font-size: 0.75rem; transition: transform 0.15s ease; }
.alerts-list { display: flex; flex-wrap: wrap; gap: var(--sp-2); margin-top: var(--sp-3); }
.alerts-item {
  background: rgba(255, 255, 255, 0.12); border: 1px solid rgba(255, 255, 255, 0.22);
  color: #fff; font-size: var(--fs-cap); padding: 0.1875rem var(--sp-2);
  border-radius: var(--r-sm); font-family: Consolas, monospace;
}
...
.main-tabs .el-tabs__header { margin-bottom: var(--sp-3); }
.main-tabs .el-tabs__item { font-size: var(--fs-strong); padding-inline: var(--sp-3); }
.main-tabs .el-tabs__nav-wrap::after { height: 1px; background: var(--line); }
...
.panel-sub { font-size: var(--fs-cap); color: var(--ink-muted); margin: calc(var(--sp-1) * -1) 0 var(--sp-3); }
...
ul.plain li::before {
  ...
  width: 0.375rem; height: 0.375rem; border-radius: 50%; background: #93c5fd;
}

.cause {
  border: 1px solid var(--line); border-left: 0.1875rem solid #93c5fd;
  border-radius: var(--r-lg);
  padding: var(--sp-3) var(--sp-4); margin-bottom: var(--sp-3);
  background: var(--surface);
  transition: box-shadow 0.15s ease;
}
.cause:hover { box-shadow: 0 0.25rem 0.875rem -0.5rem rgba(15, 23, 42, 0.18); }
.cause .title { font-size: var(--fs-body); font-weight: 600; color: #111827; }
.cause .type { font-size: var(--fs-cap); color: var(--ink-muted); margin-top: var(--sp-1); }

pre.sql {
  background: #0f172a; color: #e2e8f0;
  padding: var(--sp-3) var(--sp-4); border-radius: var(--r-md);
  font-size: var(--fs-cap); overflow-x: auto; line-height: 1.6;
  margin: var(--sp-2) 0 var(--sp-3); border: 1px solid #1e293b;
}
.sql-purpose { font-size: var(--fs-body); color: #334155; font-weight: 600; margin-top: var(--sp-1); }
ol.steps { padding-left: var(--sp-6); }
ol.steps li { font-size: var(--fs-body); line-height: 1.9; color: var(--ink-2); }
.src { font-size: var(--fs-cap); color: var(--ink-muted); margin-top: var(--sp-2); }
.hint { font-size: var(--fs-cap); color: var(--ink-muted); margin-top: var(--sp-3); line-height: 1.7; }

.foot {
  margin-top: var(--sp-6); font-size: var(--fs-cap); color: #64748b;
...
.concept { border-radius: var(--r-lg); }
.concept .body { font-size: var(--fs-body); line-height: 1.8; }
.concept .body b { color: var(--ink-2); }
...
@media (max-width: 45rem) {
  ...
  .stats { gap: var(--sp-2) var(--sp-4); }
  .main { border-radius: var(--r-lg); }
