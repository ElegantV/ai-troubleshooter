    ...
    (jobsBy[L] || []).forEach(
      (j, k) =>
        (pos['j:' + j.name] = {
          ...
          kind: 'j',
          text: j.name,
          desc: `${j.desc}（${j.schedule}）`,
        }),
    );
    (tablesBy[L] || []).forEach(
      (t, k) =>
        (pos['t:' + t.name] = { x: colX(i), y: 260 + k * 46, w: bw, h: 34, kind: 't', text: t.name, desc: t.comment }),
    );
  });
