<template>
  <div class="panel" v-if="report">
    ...
    <div v-if="report.aiAnalysis" class="ai-card">
      <div class="ai-head">
        <span class="ai-title"><el-icon class="ai-ico"><MagicStick /></el-icon>AI 综合分析</span>
        <el-tag size="small" :type="confTag(report.aiAnalysis.confidence)" effect="dark" v-if="report.aiAnalysis.confidence">{{ report.aiAnalysis.confidence }}</el-tag>
      </div>
      ...
      <div v-if="aiOpen" class="ai-detail">
        <div v-if="report.aiAnalysis.evidenceRefs.length" class="ai-sec">
          <b>依据引用：</b>
          <ul class="plain"><li v-for="(e, i) in report.aiAnalysis.evidenceRefs" :key="i">{{ e }}</li></ul>
        </div>
        <div v-if="report.aiAnalysis.verifySql.length" class="ai-sec">
          <b>建议验证 SQL：</b>
          <template v-for="(q, i) in report.aiAnalysis.verifySql" :key="i"><pre class="sql">{{ q }}</pre></template>
        </div>
        <div v-if="report.aiAnalysis.steps.length" class="ai-sec">
          <b>建议处置：</b>
          <ol class="steps"><li v-for="(s, i) in report.aiAnalysis.steps" :key="i">{{ s }}</li></ol>
        </div>
      </div>

      <div class="ai-actions" v-if="!fbDone">
        <el-button size="small" type="primary" plain :loading="fbBusy" :icon="CircleCheck" @click="confirmAi">采纳此结论</el-button>
    ...
    <template v-for="s in level2Sections" :key="s.key">
      <template v-if="s.key === 'causes' && s.causes.length">
        <h3 class="sec-h2"><el-icon class="sec-ico"><Aim /></el-icon>{{ s.title }}</h3>
      ...
      <template v-else-if="s.key === 'impact' && s.items.length">
        <h3 class="sec-h2"><el-icon class="sec-ico"><TrendCharts /></el-icon>{{ s.title }}</h3>
    ...
    <el-collapse v-if="level3Sections.length" v-model="collapsedOpen" class="level3">
      <el-collapse-item v-for="s in level3Sections" :key="s.key" :name="s.key">
        <template #title>
          <span class="sec-h3"><el-icon class="sec-ico"><component :is="secIcon[s.key]" /></el-icon>{{ s.title }}</span>
        </template>
        ...
        <template v-else-if="s.key === 'structures'">
          <div v-for="t in s.tables" :key="t.name" class="cause">
            <div class="cause-head">
              <div class="title"><el-icon class="tbl-ico"><Coin /></el-icon>{{ t.name }}</div>
              <el-tag :type="t.inKb ? 'primary' : 'info'" size="small">{{ t.inKb ? '在库' : '知识库外' }}</el-tag>
            </div>
            <div class="type">{{ t.comment }}</div>
            <el-table v-if="t.columns.length" class="cols" :data="t.columns.map(c => ({ col: c[0], type: c[1], nullable: c[2], comment: c[3] }))" size="small">
    ...
    <div v-else class="src"><el-icon class="fb-ok"><CircleCheck /></el-icon>{{ fbDone }}</div>
  </div>
</template>
...
import {
  CircleCheck, CircleClose, MagicStick, Warning, Aim, TrendCharts,
  DocumentChecked, Coin, Collection, Tools, ArrowDown,
...
const level3Sections = computed(() => props.report.sections.filter(s => ['sql', 'structures', 'refs', 'solution'].includes(s.key)));
...
async function confirmAi() {
  ...
  if (!top) { ElMessage.warning('AI 未给出可采纳的结论'); return; }
...
<style scoped>
.cause-head { display: flex; justify-content: space-between; align-items: baseline; gap: var(--sp-2); }

/* ---------- 结论（一级） ---------- */
.conclusion {
  border: 1px solid var(--line); border-left: 0.1875rem solid var(--ink-muted);
  border-radius: var(--r-lg);
  padding: var(--sp-3) var(--sp-4);
  background: var(--surface);
}
.conf-高 { border-left-color: #dc2626; }
.conf-中 { border-left-color: #d97706; }
.conf-低 { border-left-color: var(--ink-muted); }
.conclusion-head {
...
.conclusion-title {
  ...
  font-size: var(--fs-body); font-weight: 600; color: #111827;
}
.conclusion-ico { font-size: 1rem; }
.conf-高 .conclusion-ico { color: #dc2626; }
.conf-中 .conclusion-ico { color: #d97706; }
.conf-低 .conclusion-ico { color: var(--ink-muted); }
.conclusion-text {
  font-size: var(--fs-lead); line-height: 1.7; color: var(--ink);
  margin-top: var(--sp-2); font-weight: 500;
}

/* ---------- 二级 / 三级标题 ---------- */
.sec-ico { font-size: 0.9375rem; }
.tbl-ico { font-size: 0.875rem; vertical-align: -0.125rem; margin-right: 0.25rem; color: #10b981; }

/* ---------- 展开更多（候选原因 / AI 依据） ---------- */
.more-toggle, .ai-toggle {
  display: inline-flex; align-items: center; gap: var(--sp-2);
  background: none; border: none; cursor: pointer; font-family: inherit;
  font-size: var(--fs-body); color: var(--el-color-primary);
  padding: var(--sp-2) 0;
  transition: color 0.15s ease;
}
.more-toggle:hover, .ai-toggle:hover { color: var(--el-color-primary-dark-2); }
.more-chev, .ai-chev { font-size: 0.75rem; transition: transform 0.15s ease; }
.more-chev.open, .ai-chev { transform: rotate(0deg); }
.ai-toggle.open .ai-chev { transform: rotate(180deg); }
.ai-toggle { margin-top: var(--sp-1); }

/* ---------- 三级折叠区 ---------- */
.level3 { margin-top: var(--sp-1); border-top: none; border-bottom: none; }
.level3 :deep(.el-collapse-item__header) {
  border-bottom: 1px solid var(--line);
  height: auto; padding: var(--sp-3) var(--sp-1);
}
.sec-h3 {
  display: flex; align-items: center; gap: var(--sp-2);
  font-size: var(--fs-strong); color: var(--brand-deep);
}
.level3 :deep(.el-collapse-item__content) { padding-bottom: var(--sp-3); }

/* ---------- AI 卡片 / LLM 提示 ---------- */
.llm-note {
  margin-top: var(--sp-3); font-size: var(--fs-body); color: var(--ink-2);
  background: #f5f3ff; border-left: 0.25rem solid #8b5cf6;
  padding: var(--sp-2) var(--sp-3); border-radius: var(--r-sm);
  display: flex; align-items: center; gap: var(--sp-2);
}
.llm-note.warn { background: #fff7ed; border-left-color: #f59e0b; color: #92400e; }
.llm-ico { font-size: 0.9375rem; flex: none; }
.ai-card {
  margin-top: var(--sp-3); border: 1px solid #c7d2fe; border-left: 0.25rem solid #6366f1;
...
.ai-head {
  display: flex; justify-content: space-between; align-items: center;
  gap: var(--sp-2); flex-wrap: wrap;
  font-weight: 600; color: #3730a3; font-size: var(--fs-strong); margin-bottom: var(--sp-2);
}
.ai-title { display: flex; align-items: center; gap: var(--sp-2); }
.ai-ico { font-size: 1rem; }
.ai-cause { font-size: var(--fs-body); line-height: 1.7; color: #1e1b4b; }
.ai-detail {
  margin-top: var(--sp-2); padding-top: var(--sp-2);
  border-top: 1px dashed #c7d2fe;
}
.ai-sec { margin-top: var(--sp-2); font-size: var(--fs-body); color: #334155; }
.ai-sec b { color: #4338ca; }
.ai-sec .sql { margin: var(--sp-1) 0 var(--sp-2); }
.ai-actions {
  margin-top: var(--sp-3); display: flex; align-items: center;
  gap: var(--sp-3); flex-wrap: wrap;
}
.ai-actions .hint { margin-top: 0; }

.cause-高 { border-left-color: #dc2626; }
.cause-中 { border-left-color: #d97706; }
.cause-低 { border-left-color: var(--ink-muted); }
.sql-head {
  display: flex; justify-content: space-between; align-items: center;
  gap: var(--sp-2); margin-top: var(--sp-1);
}
.cols { margin-top: var(--sp-2); }

/* ---------- 反馈区 ---------- */
.fb-title { font-size: var(--fs-body); font-weight: 600; margin-bottom: var(--sp-2); }
.fb-input { display: flex; gap: var(--sp-2); margin-top: var(--sp-3); }
.fb-ok { font-size: 0.875rem; color: #059669; vertical-align: -0.125rem; margin-right: 0.25rem; }

@media (max-width: 45rem) {
  .conclusion, .ai-card { padding: var(--sp-3); }
  .fb-input { flex-wrap: wrap; }
  .fb-input .el-input { flex: 1 1 100%; }
}
</style>
