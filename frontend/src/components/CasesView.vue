<template>
  <div>
    <div class="panel">
      <h3><el-icon class="h3-ico"><DocumentAdd /></el-icon>录入已有案例</h3>
    ...
    <div class="panel">
      <h3><el-icon class="h3-ico"><Collection /></el-icon>案例列表（{{ cases.length }} 条，含人工录入与反馈沉淀）</h3>
      ...
      <el-card v-for="c in cases" :key="c.case_id" shadow="never" class="case-card">
        <template #header>
          <div class="case-head">
            ...
            <span class="case-meta">{{ c.case_id }} · {{ c.source }}{{ c.created_by ? ' · ' + c.created_by : '' }} · {{ fmtTime(c.created_at) }}</span>
          </div>
        </template>
        <div class="case-body">
        ...
        <div class="case-tags">
          ...
          <el-tag v-for="j in c.related_jobs" :key="j" size="small" type="primary" effect="plain"><el-icon class="tag-ico"><SetUp /></el-icon>{{ j }}</el-tag>
          <el-tag v-for="t in c.related_tables" :key="t" size="small" type="success" effect="plain"><el-icon class="tag-ico"><Coin /></el-icon>{{ t }}</el-tag>
        </div>
      </el-card>
    </div>
  </div>
...
async function load() {
  const [d, list] = await Promise.all([api('/dict'), api('/cases')]);
...
.tag-ico { font-size: 0.75rem; vertical-align: -0.125rem; margin-right: 0.1875rem; }

.form-actions { display: flex; flex-wrap: wrap; gap: var(--sp-2); }
.auto-note { margin-top: var(--sp-3); }
.empty {
...
.case-card:hover { box-shadow: 0 0.375rem 1.125rem -0.625rem rgba(15, 23, 42, 0.18); transform: translateY(-0.0625rem); }

.case-head {
  display: flex; justify-content: space-between; align-items: baseline;
  gap: var(--sp-2); flex-wrap: wrap;
}
.case-title { font-size: var(--fs-body); line-height: 1.5; min-width: 0; }
.case-meta { font-size: var(--fs-cap); color: var(--ink-faint); }

.case-body { display: flex; flex-direction: column; gap: var(--sp-2); }
.case-row {
...
.case-row p { color: var(--ink-2); min-width: 0; overflow-wrap: anywhere; }
